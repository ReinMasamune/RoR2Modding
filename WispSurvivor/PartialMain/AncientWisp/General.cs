...
namespace RogueWispPlugin
{
    internal partial class Main
    {
        ...
        private void AW_GetBody()
        {
            ...
            charBody.baseMaxHealth = 8000;
            charBody.levelMaxHealth = 2400;
            ...
            foreach( EntityStateMachine esm in this.AW_body.GetComponents<EntityStateMachine>() )
            {
                switch( esm.customName )
                {
                    case "Body":
                        netStates[0] = esm;
                        esm.mainStateType = new SerializableEntityStateType( typeof( AWDefaultMain ) );
                        esm.initialStateType = new SerializableEntityStateType( typeof( AWDefaultMain ) );
